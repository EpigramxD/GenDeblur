...
def grad_tv_1c(img, epsilon=1e-3):
    # x_forward
    x_f = cp(img)
    x_f[0:img.shape[0] - 1, :] = cp(img[1:, :])

    # y_forward
    y_f = cp(img)
    y_f[:, 0:img.shape[1] - 1] = cp(img[:, 1:])

    # x_backward
    x_b = cp(img)
    x_b[1:, :] = cp(img[0:img.shape[0] - 1, :])

    # y_backward
    y_b = cp(img)
    y_b[:, 1:] = cp(img[:, 0:img.shape[1] - 1])

    # x_forward_y_backward
    x_f_y_b = cp(x_f)
    x_f_y_b[:, 1:] = cp(x_f_y_b[:, 0:img.shape[1] - 1])

    # x_backward_y_forward
    x_b_y_f = cp(x_b)
    x_b_y_f[:, 0:img.shape[1] - 1] = cp(x_b_y_f[:, 1:])

    # x_mixed
    x_m = x_f_y_b - y_b
    print(f"x_m_sum: {np.sum(x_m)}")

    # y_mixed
    y_m = x_b_y_f - x_b
    print(f"y_m_sum: {np.sum(y_m)}")

    x_f -= img
    print(f"x_f_sum: {np.sum(x_f)}")

    y_f -= img
    print(f"y_f_sum: {np.sum(y_f)}")

    x_b = img - x_b
    print(f"x_b_sum: {np.sum(x_b)}")

    y_b = img - y_b
    print(f"y_b_sum: {np.sum(y_b)}")
